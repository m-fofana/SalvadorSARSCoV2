...
# I: LOAD & FORMAT DATA--------------------------------------------------------
dat2 <- read.csv("SynthData.csv", header = T, stringsAsFactors = T)

dat2 <- dat2 %>%
	mutate(inchi = as.factor(inchi)) %>%
...
dat2<- dat2 %>%
  ...
  mutate(rsexM = relevel(sexM, ref = "Male")) %>%
...
tmpvar2 <- c("casanova", "hhinc", "hsize", 
             "u18yn", "dpc")
...
The total sample includes `r nrow(dat3)` individuals with serology results, among `r length(unique(dat3$casanova))` households. 
...
varnames <- c("agecat", "sexM", "racecat", "inccat", grep("ultm$", colnames(dat3), value=T))
...
tmptab <- as.table(rowPerc(xtabs(~sintomas + covidser2, data = dat3)))[, -3] 
...
agenegF <- dat3[which(dat3$covidser2 == 0 & dat3$sexM == "Female"), c("age", "covidser2")]
agenegM <- dat3[which(dat3$covidser2 == 0 & dat3$sexM == "Male"), c("age", "covidser2")]
ageposF <- dat3[which(dat3$covidser2 == 1 & dat3$sexM == "Female"), c("age", "covidser2")]
ageposM <- dat3[which(dat3$covidser2 == 1 & dat3$sexM == "Male"), c("age", "covidser2")]
...
mdltmp1F <- gam(covidser2 ~ s(age), data = dat3[which(dat3$sexM == "Female"),])
mdltmp1M <- gam(covidser2 ~ s(age), data = dat3[which(dat3$sexM == "Male"),])
...
testdata1F = data.frame(age = seq(5, 75, by=0.2), sexM = "Female")
...
testdata1M = data.frame(age = seq(5, 75, by=0.2), sexM = "Male")
...
p2b <- ggplot(aes(x=age,y=fit, color=sexM),  data=predicts1sex) +
  geom_ribbon(aes(ymin = lower, ymax=upper, fill=sexM), alpha=0.25) +
  geom_line(aes(color=sexM), size=1.5) +
...
# Panel E
dat3$u18cat <- cut(dat3$totu18, breaks = c(0.5, 1.5, 2.5, Inf), labels = c("1", "2", "3+"))
proptable <- data.frame(adult = rep(c(1, 2), each=3)) %>%
  mutate(children = rep(c("1", "2", "3+"), 2)) %>%
  mutate(tot = NA, pos = NA, prop = NA, lower = NA, upper = NA)

for(i in 1:6){
  proptable$tot[i] <- length(dat3$covidser2[which(dat3$totad == proptable$adult[i] & dat3$u18cat == proptable$children[i])]) 
  proptable$pos[i] <- length(dat3$covidser2[which(dat3$totad == proptable$adult[i] & dat3$u18cat == proptable$children[i] & dat3$covidser2 == 1)]) 
  if(proptable$tot[i] > 0) {tmp <- prop.test(proptable[i, 4], proptable[i, 3])
  proptable$prop[i] <- tmp$estimate
  proptable[i, 6:7] <- tmp$conf.int}
  else {proptable[i, 5:7] <- NA}
...
stat.test2<-subset(dat3, totu18>=1 & totad %in% 1:2) %>%
  mutate(inter = interaction(u18cat, totad)) %>%
  # group_by(totad) %>%
...
stat.test3<-subset(adudat, !is.na(employedyn) & !is.na(dpc)) %>%
  mutate(inter = interaction(employedyn, sexM)) %>%
  group_by(employedyn) %>%
  t_test(dpc ~ sexM) %>%
  adjust_pvalue(method = "bonferroni") %>%
  add_significance("p.adj") %>%
...
p3a<-adudat %>%
  ...
  ggplot(aes(x = sexM, y = dpc, color = sexM,
                       group = sexM)) +
...
i<-1
for(i in 1:8){
  proptable$tot[i] <- length(dat3$covidser2[which(dat3$employedyn == proptable$emp[i] & dat3$sexM == proptable$sex[i] & dat3$inchi == proptable$inc[i] & dat3$age>=18)]) 
  proptable$pos[i] <- length(dat3$covidser2[which(dat3$employedyn == proptable$emp[i] & dat3$sexM == proptable$sex[i] &  dat3$inchi == proptable$inc[i] & dat3$age>=18 & dat3$covidser2 == 1)]) 
...
rownames(tmpres1) <-  c("Intercept", levels(dat3$ragecat)[-1], "Income", "Female sex", "Total residents")
...
rownames(tmpres2) <-  c("Intercept", levels(adudat$ragecat)[-(1:2)], "Female sex", "Children in household")


mdl3 <- glmer(covidser2 ~ ragecat +
              rsexM +  hsize +
                 (1 | casanova),
                 data = adudat, family = binomial, control=glmerControl(optimizer="Nelder_Mead"))
tmpse3 <- sqrt(diag(vcov(mdl3)))
tmpres3 <- cbind(OR = exp(fixef(mdl3)), LL = exp(fixef(mdl3) - 1.96 * tmpse3), UL = exp(fixef(mdl3) + 1.96 * tmpse3))
rownames(tmpres3) <-  c("Intercept", levels(adudat$ragecat)[-(1:2)], "Female sex", "Total residents in household")
